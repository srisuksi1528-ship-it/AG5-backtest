...
for i in range(50,len(df)):
    ...
    bull_trend = c > float(ema50.iloc[i]) and float(ema20.iloc[i]) > float(ema50.iloc[i]) and float(ema50.iloc[i]) > float(ema50.iloc[i-3])
    bull_rsi = 54 < float(rsi.iloc[i]) < 66
...
pf=(wb*1.5)/(lb*1.0) if lb>0 else 0
print(f"AG6 V1.5 FINAL BUY rows={len(df)} Valid Buy={vb} Sell=0 Trades={trades} Win={win:.2f}% PF={pf:.2f} Buy {wb}-{lb}")
