# AG5 V20.3 V20.6 FINAL LONG45 SHORT58 - no unpack, 5 vars separate, compile tested
...
for i in range(len(df)):
    ...
    bull_align=(retest_bull and float(close.iloc[i])>h4_fast)
    bear_align=(retest_bear and float(close.iloc[i])<h4_fast and h4_close<h4_slow and h4_fast<h4_slow)
    long_sig=bull_align and total_score>=p_thresh_long and vol_ok and (i-last_long>=3)
    short_sig=bear_align and total_score>=p_thresh_short and vol_ok and (i-last_short>=5)
...
print(f"v20.6 FINAL (ชื่อเดิม: AG5 V20.3 Scoring/BOS/Retest M15) rows={len(df)} Valid Buy={valid_buy} Sell={valid_sell} Trades={trades} Win={win:.2f}% PF={pf:.2f} Buy {w_b}-{l_b} Sell {w_s}-{l_s}")
...
with open(path,"a",newline="",encoding="utf-8") as f:
    ...
    w.writerow([today,len(df),valid_buy,valid_sell,trades,f"{win:.2f}",f"{pf:.2f}","v20.6"])
