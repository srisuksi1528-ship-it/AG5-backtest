# ELITE SMC + e-FCD V1 - Pine to Python (copy-paste ready)
...
for i in range(5, len(df)):
    ...
    bear_ob=False
    try:
        if float(low.iloc[i]) > float(high.iloc[i-2]) and float(close.iloc[i-1]) > float(open_.iloc[i-1]):
            bull_fvg=True
        if float(high.iloc[i]) < float(low.iloc[i-2]) and float(close.iloc[i-1]) < float(open_.iloc[i-1]):
            bear_fvg=True
        if float(close.iloc[i-2]) < float(open_.iloc[i-2]) and float(close.iloc[i-1]) > float(open_.iloc[i-1]) and float(close.iloc[i-1]) > float(high.iloc[i-2]) and float(vol.iloc[i-1]) > float(vol.iloc[i-2]):
            bull_ob=True
        if float(close.iloc[i-2]) > float(open_.iloc[i-2]) and float(close.iloc[i-1]) < float(open_.iloc[i-1]) and float(close.iloc[i-1]) < float(low.iloc[i-2]) and float(vol.iloc[i-1]) > float(vol.iloc[i-2]):
            bear_ob=True
    except: pass
...
print(f"ELITE V1 (ชื่อเดิม: AG5 V20.3 Scoring/BOS/Retest M15) rows={len(df)} Valid Buy={valid_buy} Sell={valid_sell} Trades={trades} Win={win:.2f}% PF={pf:.2f} Buy {w_b}-{l_b} Sell {w_s}-{l_s}")
...
with open(path,"a",newline="",encoding="utf-8") as f:
    ...
    w.writerow([today,len(df),valid_buy,valid_sell,trades,f"{win:.2f}",f"{pf:.2f}","ELITE V1"])
