# AG6 V1.0 ELITE SMC + e-FCD - FRESH FILE NO DUPLICATE
...
for i in range(5, len(df)):
    ...
    bull_fvg=bear_fvg=bull_ob=bear_ob=False
    try:
...
print(f"AG6 V1.0 ELITE (ชื่อเดิม: AG5 V20.3 Scoring/BOS/Retest M15) rows={len(df)} Valid Buy={valid_buy} Sell={valid_sell} Trades={trades} Win={win:.2f}% PF={pf:.2f} Buy {w_b}-{l_b} Sell {w_s}-{l_s}")
today=datetime.datetime.now().strftime("%Y-%m-%d")
path="HISTORY_AG6_V1_0.csv"
new=not os.path.exists(path)
with open(path,"a",newline="",encoding="utf-8") as f:
    import csv
    w=csv.writer(f)
    if new: w.writerow(["date","rows","valid_buy","valid_sell","trades","winrate","pf","code"])
    w.writerow([today,len(df),valid_buy,valid_sell,trades,f"{win:.2f}",f"{pf:.2f}","AG6 V1.0"])
